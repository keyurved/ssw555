...
def validate(self):
    ...
    def _check_dates(self):
        ...
        if self.husband is not None and self.wife is not None:
            ...
            if self.married_date is not None and self.husband.alive is not None:
                if self.married_date < self.husband.alive:
                    raise ValueError("Married date %s cannot be before the husband's birth date %s" % (self.married_date.strftime('%Y-%m-%d'), self.husband.alive.strftime('%Y-%m-%d')))
                if self.married_date < self.wife.alive:
                    raise ValueError("Married date %s cannot be before the wife's birth date %s" % (self.married_date.strftime('%Y-%m-%d'), self.wife.alive.strftime('%Y-%m-%d')))
